...
function init() {
    ...
    for (i = 0; i < elems.length; i++) {
        item = elems[i];
        var type = item.tagName.toLowerCase(), options = {};
    ...
    for (var type in eventDelegatorObj) {
        (function (eventName) {
            ...
            if (root.bindedEvent[eventName] !== true) {
                var fixEventName = eventName;
                if (eventName === 'mouseenter') {
                    fixEventName = 'mouseover';
                } else if (eventName === 'mouseleave') {
                    fixEventName = 'mouseout';
                }
                root.addEventListener(fixEventName, function (e) {
                    var parent = e.target;
                    function findCB(parent) {
                        ...
                        if (!!cb) {
                            ...
                            if (!related || target != related && !domContains(target, related)) {
                                var eProps = ['altKey','bubbles','button','buttons','cancelable','clientX','clientY','ctrlKey','metaKey','offsetX','offsetY','screenX','screenY','shiftKey','which'];
                                var newEvent = new DomEvents(e);
                                eProps.forEach(function (key) {
                                    newEvent[key] = e[key];
                                });
                                newEvent.currentTarget = parent;
                                newEvent.type = eventName;
                                newEvent.timeStamp = e.timeStamp || new Date().getTime();
